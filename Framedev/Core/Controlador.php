...
class Controlador {
    ...
    {
        $directory = new \RecursiveDirectoryIterator(URL_MODELO);
        $iterator = new \RecursiveIteratorIterator($directory);
        $execute = false;
        foreach ($iterator as $info) {
          if (is_file($info))
            if($info->getBasename('.php') == $database){
              $path = substr($info->getPathname(), strlen(URL_MODELO), 256);
              $filename = $info->getFilename();
              $execute = true;
            }
        }
        if($execute){
          require URL_MODELO . $path;
          $object = $database.'Elq';
          return new $object();
        }
    }
    ...
    public function loadMiddleware($middleware)
    {
        ...
        $directory = new \RecursiveDirectoryIterator(URL_MIDDLEWARE);
        $iterator = new \RecursiveIteratorIterator($directory);
        $execute = false;
        foreach ($iterator as $info) {
          if (is_file($info))
            if($info->getBasename('.php') == $middleware){
              $path = substr($info->getPathname(), strlen(URL_MIDDLEWARE), 256);
              $filename = $info->getFilename();
              $execute = true;
            }
        }
